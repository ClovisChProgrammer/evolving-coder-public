...
Configure automatic evolving-coder triggers for AI coding agents.
    ...
    "UserPromptSubmit": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "./skills/evolving-coder/scripts/activator.sh"
          }
        ]
      }
    ],
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "./skills/evolving-coder/scripts/error-detector.sh"
          }
        ]
      }
    ]
    ...
    "UserPromptSubmit": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "~/.claude/skills/evolving-coder/scripts/activator.sh"
          }
        ]
      }
    ]
...
Consider logging the learning to `.learnings/` using the format from the evolving-coder skill.
...
3. Send any prompt
4. Verify you see `<evolving-coder-reminder>` in the context
...
```bash
./skills/evolving-coder/scripts/extract-skill.sh test-skill --dry-run
```
...
```bash
chmod +x ./skills/evolving-coder/scripts/activator.sh
chmod +x ./skills/evolving-coder/scripts/error-detector.sh
chmod +x ./skills/evolving-coder/scripts/extract-skill.sh
```
...
```json
{
  "command": "/absolute/path/to/skills/evolving-coder/scripts/activator.sh"
}
```
